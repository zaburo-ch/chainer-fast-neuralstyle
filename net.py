...
class ResidualBlock(chainer.Chain):
    def __init__(self, n_in, n_out, stride=1, ksize=3):
        ...
        with self.init_scope():
            self.c1 = L.Convolution2D(n_in, n_out, ksize, stride, 1, w)
            self.c2 = L.Convolution2D(n_out, n_out, ksize, 1, 1, w)
            self.b1 = L.BatchNormalization(n_out)
            self.b2 = L.BatchNormalization(n_out)
...
class FastStyleNet(chainer.Chain):
    def __init__(self):
        ...
        with self.init_scope():
            self.c1 = L.Convolution2D(3, 32, 9, stride=1, pad=4)
            self.c2 = L.Convolution2D(32, 64, 4, stride=2, pad=1)
            self.c3 = L.Convolution2D(64, 128, 4,stride=2, pad=1)
            self.r1 = ResidualBlock(128, 128)
            self.r2 = ResidualBlock(128, 128)
            self.r3 = ResidualBlock(128, 128)
            self.r4 = ResidualBlock(128, 128)
            self.r5 = ResidualBlock(128, 128)
            self.d1 = L.Deconvolution2D(128, 64, 4, stride=2, pad=1)
            self.d2 = L.Deconvolution2D(64, 32, 4, stride=2, pad=1)
            self.d3 = L.Deconvolution2D(32, 3, 9, stride=1, pad=4)
            self.b1 = L.BatchNormalization(32)
            self.b2 = L.BatchNormalization(64)
            self.b3 = L.BatchNormalization(128)
            self.b4 = L.BatchNormalization(64)
            self.b5 = L.BatchNormalization(32)
